...
if __name__ == '__main__':
    ...
    df = lambda x: ((2 * math.exp(-x) - 2 * x * math.exp(-x) + (4 * x) / (2 * x**2)) * (2 * x**3 + 2 * x**2 - 3 * x - 5)) + \
                ((2 * x * math.exp(-x) + math.log(abs(2 * x**2) + 1e-10)) * (6 * x**2 + 4 * x - 3))
    ...
    while b <= 1.5:
        if f(a) * f(b) < 0:  # Sign change detected
            ...
            print(f"The equation f(x) has an approximate root at x = {root:.9f}")
        a += 0.1
        b = a + 0.1
